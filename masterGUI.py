...
def gibbsCalculate():
    ...
    ##tk.Label(master, text = str(tempgmm)).grid(row = 5, column = 1)
    ##tk.Label(master, text = 'kJ/mol').grid(row = 5, column = 2)
    
    ## Calculating the ideal part at 0.5 composition
    tempgid = round(R * T * ((1 - tempxB) * math.log(1 - tempxB) + tempxB * math.log(tempxB)), 3)
    ##tk.Label(master, text = str(tempgid)).grid(row = 6, column = 1)
    ##tk.Label(master, text = 'kJ/mol').grid(row = 6, column = 2)
    ...
    return g0ElemA, g0ElemB, composition, gmm, gid
    
def gibbsEqui():
    g0ElemA, g0ElemB, composition, gmm, gid = gibbsCalculate()
    T = float(temp.get())
    R = 8.314
    tempxB = 0.5
    ## Calculating the mechanical mixing part at 0.5 composition
...
def gibbsTotal():
    g0ElemA, g0ElemB, composition, gmm, gid = gibbsCalculate()
    T = float(temp.get())
    gTotal = [gmm[i] + gid[i] for i in range(len(gmm))]
    figure1 = Figure(figsize = (5, 4), dpi = 100)
    plot1 = figure1.add_subplot(1, 1, 1)
...
## Calculate button
calc = tk.Button(master, text = 'Calculate', bg = 'silver', height = 2, width = 10, font = ('Times 14 bold'), command = gibbsEqui).grid(row = 3, column = 0)

## Plot buttons
gmmPlot = tk.Button(master, text = 'Plot\nMech part', bg = 'orange', height = 2, width = 10, font = ('Times 10 bold'), command = gibbsMech).grid(row = 3, column = 1)
gidPlot = tk.Button(master, text = 'Plot\nIdeal part', bg = 'orange', height = 2, width = 10, font = ('Times 10 bold'), command = gibbsId).grid(row = 3, column = 2)
gexPlot = tk.Button(master, text = 'Plot\nExcess part', bg = 'orange', height = 2, width = 10, font = ('Times 10 bold')).grid(row = 3, column = 3)
gTotalPlot = tk.Button(master, text = 'Plot\nTotal', bg = 'orange', height = 2, width = 10, font = ('Times 10 bold'), command = gibbsTotal).grid(row = 3, column = 4)

## Quit button
quitButton = tk.Button(master, text = 'Quit', bg = 'silver', height = 2, width = 10, font = ('Tomes 14 bold'), command = quitButtonFunction).grid(row = 0, column = 2)

## Gibbs energy displayer
tk.Label(master, text = 'Gibbs energy at equiatomic composition', font = ('Times 12 bold')).grid(row = 4, column = 0)
tk.Label(master, text = 'G_mm').grid(row = 5, column = 0)
tk.Label(master, text = 'G_id').grid(row = 6, column = 0)
